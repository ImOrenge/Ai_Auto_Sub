...
type FilterStatus = "all" | "pending" | "processing" | "done" | "error" | "canceled";
...
function JobsPageContent() {
    ...
    const [counts, setCounts] = useState<StatusCounts>({ all: 0, pending: 0, processing: 0, done: 0, error: 0, canceled: 0 });
    ...
    const fetchJobs = useCallback(async (options?: { showRefresh?: boolean }) => {
        ...
        try {
            ...
            setJobs(data.jobs || []);
            setCounts(data.counts || counts); // Keep old counts if missing
    ...
    useEffect(() => {
        fetchJobs();
        const interval = setInterval(() => fetchJobs({ showRefresh: false }), 5000); // 5s polling
    ...
    return (
        <PageContainer className="gap-6 py-8 pb-32">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-semibold tracking-tight">Job Console</h1>
                    <p className="text-sm text-muted-foreground">Manage and monitor processing tasks.</p>
                </div>
